...
def registro(request):
    ...
    if request.method == "GET" and "validar_username" in request.GET:
...
def lista_usuarios(request):
    ...
    empleados = User.objects.select_related(
        'perfilemple'
    ).filter(
        perfilemple__isnull=False
    )
...
def Usuarios(request):
    ...
    total_empleados = User.objects.filter(
        perfilemple__isnull=False
    ).count()
    ...
    top_compradores = _top_compradores(
        limite=5
    )
...
def api_estadisticas_usuarios(request):
    ...
    GET /api/estadisticas-usuarios/?top=10
    """
    ...
    top_compradores_qs = _top_compradores(
        limite=limite_top
    )
