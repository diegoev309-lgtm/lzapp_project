...
from dashboard.models import Producto, DescuentoAsignado, Venta, DetalleVenta, Pedido
...
def _construir_preference_data(request, lat=None, lng=None, direccion=None):
    ...
    return {
        ...
        "metadata": {
            ...
            # Mismo mecanismo: el webhook (llamado por MP, sin la sesión del
            # cliente) necesita esto para poder guardarlo en el Pedido que
            # se crea recién ahí -- antes de esto, la ubicación nunca
            # llegaba a ningún lado más allá del formulario.
            "cliente_latitud": lat,
            "cliente_longitud": lng,
            ...
            if not Venta.objects.filter(mp_payment_id=payment_id).exists():
                ...
                if usuario and items_comprados:
                    with transaction.atomic():
                        ...
                        # El Venta.save() de arriba ya disparó la señal que
                        # crea el Pedido (dashboard/models.py:
                        # crear_pedido_automatico) -- acá se completa con la
                        # ubicación que viajó en el metadata del pago, ya
                        # que el webhook no tiene la sesión del cliente para
                        # leerla de ahí. Antes de esto, estos 3 campos
                        # quedaban siempre en NULL.
                        cliente_latitud = metadata.get('cliente_latitud')
                        cliente_longitud = metadata.get('cliente_longitud')
                        direccion_entrega = metadata.get('direccion_entrega')
                        if cliente_latitud and cliente_longitud:
                            pedido = Pedido.objects.filter(venta=venta).first()
                            if pedido:
                                pedido.cliente_latitud = cliente_latitud
                                pedido.cliente_longitud = cliente_longitud
                                pedido.direccion_entrega = (direccion_entrega or '')[:255]
                                pedido.save(update_fields=[
                                    'cliente_latitud', 'cliente_longitud', 'direccion_entrega',
                                ])
